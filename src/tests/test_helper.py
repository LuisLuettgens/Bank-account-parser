...
def test_is_valid_csv_file(tmp_path):
    ...
    assert helper.is_valid_csv_file(correct_file) == True
    
    # Test 4: file is csv (upper cases)
    correct_file2 = str(test_CSV)
    
    assert helper.is_valid_csv_file(correct_file2) == True
...
class TestHelper(unittest.TestCase):
    ...
    def test_is_valid_csv_file(self):
        ...
        with self.assertRaisesRegex(expected_exception=ValueError,
                                    expected_regex='The input file does not appear to be a csv file. Please check the file-suffix.'):
            helper.is_valid_csv_file(not_a_csv_file_exeption)
